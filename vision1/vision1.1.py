...
class VisionNode(Node):
    def __init__(self):
        ...
        self.obs_pub = self.create_publisher(ObjectDetection, '/obstacle_detections', 10)
        self.scene_pub = self.create_publisher(String, '/clip_scene', 10)
        self.get_logger().info('Iniciando Vision Node...')

    def run(self):
        global ultima_descripcion_clip, clip_bloqueado, lock, frames_vacio_consecutivos
        
        # 1. CONFIGURACIÓN DE MODELOS 
        device = "cuda" if torch.cuda.is_available() else "cpu"
        pkg_path = get_package_share_directory('rsi_package')

        # MODELO DE YOLO: 
        model_yolo = YOLO(os.path.join(pkg_path, 'models', 'best_YOLO_V4.pt')) 
        ...
        # modelo CLIP 
        model_clip.load_state_dict(torch.load(os.path.join(pkg_path, 'models', 'best_clip_v7.pt'), map_location=device, weights_only=True))
        ...
        clases_clip = [
           ...
           "part_circular_white_assembled", "part_circular_white_dissasembled", "part_circular_yellow_assembled", "part_circular_yellow_dissasembled",
        ...
        self.get_logger().info(f"Sistema iniciado en {device.upper()}. Presiona q para salir.")
        ...
        try:
            while rclpy.ok():
                ...
                # Publicar métricas generales de ROS (Conteo y etiquetas)
                object_count = 0
                labels = []
                for r in results:
                    ...
                    if r.boxes is not None:
                        ...
                        for box in r.boxes:
                            labels.append(model_yolo.names[int(box.cls)])
                ...
                for r in results:
                    if r.masks is not None:
                        for i in range(len(r.masks.xy)):
                            ...
                            # --- PUBLICACIÓN EN TÓPICOS DE ROS PARA CADA OBJETO ---
                            # Original Topic
                            msg = Float64MultiArray()
                            msg.data = [float(cX), float(cY), float(distancia_m)]
                            self.centroid_pub.publish(msg)

                            # Selective Tracking Topic
                            det = ObjectDetection()
                            det.label = label_yolo
                            det.cx = float(cX)
                            det.cy = float(cY)
                ...
                for c in contornos:
                    ...
                    if len(valores_reales) > 0:
                        ...
                        if altura_real_objeto_m > ALTURA_MAX_ENSAMBLE_M:
                            ...
                            # Publicar obstaculo en topic
                            obs_msg = ObjectDetection()
                            obs_msg.label = "obstacle"
                            obs_msg.cx = float(ox + ow//2)
                            obs_msg.cy = float(oy + oh//2)
                            obs_msg.depth = float(distancia_obstaculo_m)
                ...
                with lock:
                    ...
                    self.scene_pub.publish(String(data=escena_actual))
