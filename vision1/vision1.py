...
# RUTAS DE MODELOS
# Ajusta estas rutas a donde tengas los archivos .pt
MODELS_DIR = "models"
YOLO_MODEL_PATH = os.path.join(MODELS_DIR, "best_YOLO_V4.pt")
CLIP_MODEL_PATH = os.path.join(MODELS_DIR, "best_clip_v7.pt")
...
def run():
    ...
    # modelo CLIP
    model_clip.load_state_dict(torch.load(CLIP_MODEL_PATH, map_location=device, weights_only=True))
    ...
    depth_scale = depth_sensor.get_depth_scale()  # Obtiene la escala de la cámara para convertir los datos a metros
    ...
    print(f"Sistema iniciado en {device.upper()}. Presiona q para salir.")
    ...
    try:
        while True:
            ...
            # Métricas generales (conteo y etiquetas)
            object_count = 0
            labels = []
            for r in results:
