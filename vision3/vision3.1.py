...
class VisionNode(Node):
    ...
    def publish_custom_detections(self, custom_dets, depth_matrix, depth_scale, frame_w, frame_h):
        """Publica conteo, labels y detecciones (centroid + ObjectDetection) de piezas reales."""
        object_count = len(custom_dets)
        labels = [d["label"] for d in custom_dets]
        ...
        for d in custom_dets:
            ...
            distancia_m = get_distancia_centroide(
                d["bbox"], depth_matrix, depth_scale, frame_w, frame_h
            )
    ...
    def publish_obstacle_detections(self, base_dets, depth_matrix, depth_scale, frame_w, frame_h):
    ...
    def publish_clip_detections(self, resultados_clip, custom_dets, depth_matrix, depth_scale, frame_w, frame_h):
        """Publica, por cada pieza clasificada por CLIP, su detección (label CLIP + distancia)."""
        for r in resultados_clip:
            ...
            cX, cY = (x1 + x2) // 2, (y1 + y2) // 2
            distancia_m = get_distancia_centroide(
                det_actual["bbox"], depth_matrix, depth_scale, frame_w, frame_h
            )

            det = ObjectDetection()
            det.label = r["label_clip"]
            det.cx = float(cX)
            det.cy = float(cY)
            det.depth = float(distancia_m)
            self.det_pub.publish(det)
...
def dibujar_barra_superior(frame, resultados_clip, fps):
    ...
    return frame, n_assembled, n_dissasembled
...
def main(args=None):
    ...
    try:
        while rclpy.ok():
            ...
            #Publicación ROS2 (piezas y obstáculos crudos de YOLO)
            vision_node.publish_custom_detections(
                custom_dets, depth_matrix, depth_scale, frame_w, frame_h
            )
            vision_node.publish_obstacle_detections(
                base_dets_filtered, depth_matrix, depth_scale, frame_w, frame_h
            )
            ...
            #Publicación ROS2 (clasificación CLIP por pieza)
            vision_node.publish_clip_detections(
                resultados_clip_actuales, custom_dets, depth_matrix, depth_scale, frame_w, frame_h
            )

            annotated = dibujar_piezas_clip(annotated, resultados_clip_actuales, custom_dets, depth_matrix, depth_scale)
            annotated, n_assembled, n_dissasembled = dibujar_barra_superior(annotated, resultados_clip_actuales, fps)

            #Publicación ROS2 (resumen de escena: piezas ensambladas / desensambladas)
            vision_node.publish_scene(f"assembled:{n_assembled} dissasembled:{n_dissasembled}")
